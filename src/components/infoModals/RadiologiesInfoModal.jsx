            ...
            <div className="d-flex flex-xxl-nowrap flex-wrap">
              <div className="contact-info w-100">
                ...
                <div className="modal-body">
                      ...
                      {modalInfo?.doctorsAppointments?.instructions && modalInfo?.doctorsAppointments?.instructions.map((el,i)=>(
                              <li key={i} style={{fontWeight:'normal',listStyle:'inside'}}>{el}</li>
                              ))
                            }  
                      </ul>
                      <ul className='fw-bold'>Դեղամիջոցներ
                      {modalInfo?.doctorsAppointments?.medicine && modalInfo?.doctorsAppointments?.medicine.map((el,i)=>(
                              <li key={i} style={{fontWeight:'normal',listStyle:'inside'}}>{el}</li>
                              ))
                      }  
                      </ul>
                      <ul className='fw-bold'>Հետազոտություններ
                      {modalInfo?.doctorsAppointments?.researches && modalInfo?.doctorsAppointments?.researches.map((el,i)=>(
                              <li key={i} style={{fontWeight:'normal',listStyle:'inside'}}>{el}</li>
                              ))
                            }  
                      </ul>
